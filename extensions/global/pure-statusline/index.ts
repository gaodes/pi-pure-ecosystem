...
const DEFAULT_LINES: ConfigRow[] = [
	{
		left: ["persona", "separator", "model", "separator", "path2", "separator", "path1", "git"],
		right: ["context_pct"],
	},
...
function renderSegment(id: SegmentId, ctx: RenderCtx): Seg {
	...
	switch (id) {
		case "persona": {
			...
			const icon = i ? `${i} ` : "";
			const styled = cc((seg.style as string) || "persona", `${icon}${personaId}`);
			return { content: theme.bold(styled), visible: true };
		}
		case "pi": {
		...
		case "path2": {
			const pwd = data.cwd;
			const parent = dirname(pwd);
			return { content: cc((seg.style as string) || "text", withIcon(i, basename(parent))), visible: true };
		}
		case "git": {
