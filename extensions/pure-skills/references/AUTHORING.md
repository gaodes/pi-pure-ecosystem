...
Format: `CLI: tool1>=version, tool2>=version, ...` — keep it under 500 chars total. The meta-skill extracts these during validation and tracks versions in `.upstream.json` for change detection.
...
- `.upstream.json` — only for imported skills. Tracks the inspiration source.
