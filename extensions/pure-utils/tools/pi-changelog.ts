// Originally from @aliou/pi-dev-kit via extensions/pi-devkit/tools/pi-changelog.ts
...
interface ParsedChangelog {
	entries: Array<{ version: string; content: string; lineStart: number; lineEnd: number }>;
}
...
function findChangelogEntry(changelogContent: string, requestedVersion?: string): ChangelogEntry {
	...
	if (requestedVersion) {
		...
		throw new Error(`Version ${requestedVersion} not found. Available versions: ${allVersions.join(", ")}`);
	}
...
async function fetchGithubChangelog(): Promise<string> {
	...
	if (!res.ok) {
		throw new Error(`Failed to fetch changelog from GitHub: ${res.status} ${res.statusText}`);
	}
...
function renderChangelogContent(content: string, theme: Theme, maxLines?: number): string[] {
...
function isPartial(options: ToolRenderResultOptions): boolean {
	return (options as { isPartial?: boolean }).isPartial ?? false;
}
...
pi_changelog // Get latest changelog`,
		...
		async execute(
			...
			if (params.version && isNewerThanInstalled(params.version)) {
				...
				return {
					...
					details: { changelog, source: "github" },
				};
			}
			...
			return {
				content: [{ type: "text", text: message }],
				details: { changelog, source: "local" },
			};
		},
		...
		renderResult(result: AgentToolResult<ChangelogDetails>, options: ToolRenderResultOptions, theme: Theme) {
			...
			if (options.expanded) {
				lines.push(theme.fg("accent", theme.bold(`Version: ${details.changelog.version}`)), "");
				lines.push(...renderChangelogContent(details.changelog.content, theme));
				fields.push(new Text(lines.join("\n"), 0, 0));
			} else {
				lines.push(theme.fg("accent", theme.bold(`Version: ${details.changelog.version}`)), "");
				lines.push(...renderChangelogContent(details.changelog.content, theme, COLLAPSED_LINES));
	...
	pi.registerTool<typeof ChangelogVersionsParamsSchema, ChangelogVersionsDetails>({
		...
		renderResult(result: AgentToolResult<ChangelogVersionsDetails>, options: ToolRenderResultOptions, theme: Theme) {
			const { details } = result;
			if (!details?.versions) {
				const text = result.content[0];
				return new Text(text?.type === "text" && text.text ? text.text : "No result", 0, 0);
			...
			const fields: Array<
				{ label: string; value: string; showCollapsed?: boolean } | (Component & { showCollapsed?: boolean })
			> = [];
			const lines: string[] = [theme.fg("accent", `${details.versions.length} versions available:`), ""];
			const cols = 6;
			const maxLen = Math.max(...details.versions.map((version) => version.length));
