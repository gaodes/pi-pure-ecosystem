// Originally from @aliou/pi-dev-kit via extensions/pi-devkit/tools/detect-package-manager.ts
...
import { ToolCallHeader } from "../ui/components";
	...
	pi.registerTool<typeof Params, PackageManagerDetails>({
		...
		renderResult(
			...
			if (!details?.packageManager) {
				...
				const errorMessage = text?.type === "text" && text.text ? text.text : "Failed to detect package manager";
