...
import { ToolBody, ToolCallHeader, ToolFooter } from "./ui";
...
async function fetchGithubChangelog(): Promise<string> {
	try {
		...
		throw new Error(`Failed to fetch changelog from GitHub: ${error instanceof Error ? error.message : String(error)}`);
	}
}
