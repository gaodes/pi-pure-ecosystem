...
export interface TabbedScrollablePanelOptions extends Omit<ScrollablePanelOptions, "buildContent"> {
	tabs: PanelTab[];
}
...
export class TabbedScrollablePanel implements Component {
	...
	handleInput(data: string): boolean {
		...
		if (matchesKey(data, "shift+tab")) {
			this.activeTab = (this.activeTab - 1 + this.options.tabs.length) % this.options.tabs.length;
			this.scrollOffset = 0;
			this.invalidate();
			this.tui.requestRender();
			return true;
	...
	render(width: number): string[] {
		...
		lines.push(truncateToWidth(` ${this.theme.fg("accent", this.theme.bold(this.options.title))}`, width));
		lines.push(this.renderTabBar(width));

		if (tab?.subtitle) {
			lines.push(truncateToWidth(`  ${this.theme.fg("dim", tab.subtitle)}`, width));
		} else {
			lines.push("");
		}
		...
		if (this.scrollOffset > 0) {
			lines.push(truncateToWidth(this.theme.fg("dim", `  ↑ ${this.scrollOffset} lines above`), width));
		} else {
			lines.push("");
		}
		...
		if (remaining > 0) {
			lines.push(truncateToWidth(this.theme.fg("dim", `  ↓ ${remaining} lines below`), width));
		} else {
			lines.push("");
		}
