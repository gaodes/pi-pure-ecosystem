...
export function renderInfoBox(title: string, body: string, width: number, theme: Theme): string[] {
	if (width < 10) {
	...
	const rows = bodyLines.map((line) => {
		...
		return `${theme.fg("borderMuted", "│")} ${line}${" ".repeat(padLen)} ${theme.fg("borderMuted", "│")}`;
	});

	const bottom = theme.fg("borderMuted", `└${"─".repeat(Math.max(0, width - 2))}┘`);

	return [top, ...rows, bottom];
}

export function renderInfoBoxLines(title: string, lines: string[], width: number, theme: Theme): string[] {
	if (width < 10) {
		return [truncateToWidth(title, width)];
	}
