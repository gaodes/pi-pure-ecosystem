...
export class ToolCallSummaryField<TCall extends { status: "running" | "done" | "error" }> implements Component {
	constructor(
	...
	render(width: number): string[] {
		...
		const th = this.theme;
		const names = this.toolCalls.map((toolCall) => this.formatter(toolCall).label);
