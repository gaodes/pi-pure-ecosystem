...
export class FailedToolCallsField<TCall extends { status: "running" | "done" | "error"; error?: string }>
	implements Component
{
	...
	render(width: number): string[] {
		...
		const failed = this.toolCalls.filter((toolCall) => toolCall.status === "error");
