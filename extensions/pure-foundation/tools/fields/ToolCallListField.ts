...
export class ToolCallListField<TCall extends { status: "running" | "done" | "error" }> implements Component {
	constructor(
	...
	render(width: number): string[] {
		...
		for (const toolCall of this.toolCalls) {
			const indicator =
				toolCall.status === "running" ? " " : toolCall.status === "done" ? th.fg("success", "✓") : th.fg("error", "✗");
