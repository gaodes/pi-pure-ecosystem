...
export class ToolLlmTelemetryFooter implements Component {
	...
	render(width: number): string[] {
		...
		const failedCalls = toolCalls?.filter((toolCall) => toolCall.status === "error").length ?? 0;

		parts.push(failedCalls > 0 ? `${totalCalls} calls (${failedCalls} failed)` : `${totalCalls} calls`);
...
function getCostSummary(
	...
	const hasCurrencyBuckets = usage?.toolCostUsd !== undefined || usage?.toolCostEur !== undefined;

	if (hasCurrencyBuckets) {
		const usd = usage?.totalCostUsd ?? (usage?.llmCost ?? 0) + (usage?.toolCostUsd ?? 0);
	...
	for (const toolCall of toolCalls ?? []) {
		...
		const details = (result as { details?: { cost?: number; costCurrency?: "USD" | "EUR" } }).details;
